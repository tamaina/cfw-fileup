...
# 3. 自分のPR/Issueで人間のレビューを拾う
自分が提出しているPR及び関連Issueのコメントを取得し、もし人間がレビューでコード修正やコメントで実装方針の変更を提案していた場合、自分で解決せずに、 サブエージェント pr-reviewer & pr-patcher や feature-implementer を適宜呼び出して作業する。
...
# 前回のセッションが途中で終わる場合
前回のルーティンセッションが何らかの理由で途中で停止する可能性があります。  
毎回のルーティンで、1, 2は必ず行うようにしてください。  
3, 4の確認段階で、実装の未完了がないかよく確認し、もしあれば優先的に対応してください。
