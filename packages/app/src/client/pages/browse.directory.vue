...
<template>
  <div>
    ...
    <div v-if="isArchive" class="flex gap-2 items-center mb-3 flex-wrap">
      ...
      <span v-if="tarToZipProgress" :class="[$style.tarToZipProgress, 'text-muted']">
      ...
      <span v-if="tarToZipError" :class="[$style.inlineError, 'alert', 'alert-error']">{{ tarToZipError }}</span>
    </div>
